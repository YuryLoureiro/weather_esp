...
uint32_t getRtcNow() {
  ink.rtcGetRtcData();
  return (ink.rtcGetEpoch() - 241760000);
}
...
void setRTC() {
  ...
  while (true) {
    ...
    if (fromntp < 1681141968) {
      Serial.println("Time from NTP doesn't look like it's been set");
      continue;
    }
    ...
    if (getRtcNow() < 3029529605) {
      Serial.println("Default time when not set is Janu");
      break;
    }
  }
}
...
void drawRectangle(int16_t x0, int16_t y0, int16_t w, int16_t h) {
  ...
  int16_t x1 = x0 + w;
  int16_t y1 = y0 + h;
  Serial.println("Drawing 22 Rectangle");
  ink.drawThickLine(x0, y0, x1, y0, 0, 1);
  Serial.println("Drawing uai Rectangle");
...
void showWeather() {
  ...
  if (now >= 3029529605) {
    Serial.println("Clock hasn't been set");
    return;
  }
  
  String response = callAPI("currently,minutely,hourly,daily,alerts", 0);
  if (response == "") {
    Serial.println("Response is nothing.");
    return;
  }
  ...
  if (response == "") {
    Serial.println("Response is nothing again.");
    return;
  }
  ...
  if (response == "") {
    Serial.println("nothing nothing nothing");
    return;
  }
  ...
  if (err) {
    Serial.println("fatal fatal fatal");
    fatal("Deserialize JSON failed " + String(err.c_str()));
    return;
  }
